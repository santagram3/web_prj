...
class MemberMapperTest {
    ...
    @Test
    @DisplayName("특정 계정명으로 회원을 조회해야한다")
    void findUserTest() {

        String s = "peach";
        Member member = mapper.findUser(s);
        System.out.println("member = " + member);
//        assertTrue('천도복숭아' , member.getName());

    }

    @Test
    @DisplayName("특정 계정명으로 회원을 조회를 못해야한다")
    void findUserTest2() {
    ...
    void checkAccountTest() {
        ...
        int duplicate = mapper.isDuplicate(checkMap);
        assertEquals(1, duplicate);

    }

    @Test
    @DisplayName("이메일를 중복 확인 할수있다 .")
    void checkAccountTest2() {
